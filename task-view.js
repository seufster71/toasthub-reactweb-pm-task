...
export default function PMTaskView({containerState, itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption,  
	closeModal, inputChange, goBack, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.PM_TASK_PAGE != null) {
    	columns = itemState.prefLabels.PM_TASK_PAGE;
    }
    let group = "TABLE1";
	...
	if (itemState.prefTexts.PM_TASK_PAGE != null && itemState.prefTexts.PM_TASK_PAGE.PM_TASK_PAGE_HEADER != null) {
		header = itemState.prefTexts.PM_TASK_PAGE.PM_TASK_PAGE_HEADER.value;
	}
